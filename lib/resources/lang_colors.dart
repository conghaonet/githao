...
const Map<String, Map<String, String>> LANG_COLORS = {
  "1C Enterprise": {
    "color": "#814CCC",
...
class LangColors {
  Map<String, Map<String, String>> _colors = HashMap<String, Map<String, String>>(equals: (a, b) => a.toUpperCase() == b.toUpperCase(),
      hashCode: (a) => a.toUpperCase().hashCode);

  Map<String, Map<String, String>> get colors => _colors;
  static final LangColors _langColors = LangColors._internal();

  LangColors._internal() {
    _colors.addAll(LANG_COLORS);
  }

  factory LangColors() => _langColors;

  Color getColor(String language) {
    try {
      if(colors.containsKey(language) && colors[language]['color'] != null) {
        String strColor = colors[language]['color'];
        if(strColor.startsWith('#') && strColor.length == 7) {
          ...
          return Color.fromARGB(0xff, int.parse('0x$strR'), int.parse('0x$strG'), int.parse('0x$strB'));
        }
      }
    } catch(e) {
      print(e.toString());
  ...
  String getLanguageTrendingUrl(String language) {
    if(colors.containsKey(language) && colors[language]['url'] != null) {
      String url = colors[language]['url'];
...
LangColors langColors = LangColors();
